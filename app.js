    ...
    element.innerHTML = `
      <div style="font-family: 'Inter', sans-serif; max-width: 210mm; margin: 0 auto; background: white; padding: 20px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; padding: 30px; background: #f8fafc; border-bottom: 1px solid #e2e8f0;">
          ...
          <div>
            <div style="font-size: 12px; color: #64748b;">Projeto</div>
                ...
                ${items.map(s => {
                  ...
                  return `
                    <tr>
                      ...
                      <td style="padding: 8px 12px; text-align: center; border-bottom: 1px solid #e2e8f0;">${s.qty}</td>
                      <td style="padding: 8px 12px; text-align: right; border-bottom: 1px solid #e2e8f0;">R$ ${price.toFixed(2)}</td>
                      <td style="padding: 8px 12px; text-align: right; border-bottom: 1px solid #e2e8f0;">R$ ${total.toFixed(2)}</td>
                    </tr>
                  `;
                }).join('')}
              `).join('')}
...
// Variável para controlar a inicialização dos gráficos
...
function destroyCharts() {
  if (app.charts) {
    ...
    if (app.charts.revenue) {
      ...
      app.charts.revenue = null;
    }
  }
}

// Função para inicializar gráficos (chamada apenas quando necessário)
...
// Função auxiliar para receita dos últimos 6 meses
app.getLast6MonthsRevenue = function() {
  const labels = [];
  const values = [];
  const today = new Date();
...
// Sobrescrever o método updateDashboard para evitar chamadas múltiplas
...
// Adicione esta função dentro do objeto app, antes do final
initCharts: function() {
  setTimeout(() => {
    try {
...
// Função auxiliar para receita dos últimos 6 meses
getLast6MonthsRevenue: function() {
  const labels = [];
  const values = [];
  const today = new Date();
